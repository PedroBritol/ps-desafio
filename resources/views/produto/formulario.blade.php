...
<div class="row">
    ...
    <div>
        <input type="text" id="nome" name="nome" value="{{ isset($produto) ? $produto->name : old('nome') }}"
...
<div class="row">
    ...
    <div>
        <textarea id="descricao" name="descricao" class="form-control @error('descricao') is-invalid @enderror"
            ...
            required>{{ isset($produto) ? $produto->description : old('descricao') }}</textarea>
        @error('descricao')
            <span class="invalid-feedback" role="alert">
                <i class="fi-circle-cross"></i><strong> {{ $message }}</strong>
            </span>
...
<div class="row">
    ...
    <div>
        <input type="text" id="preco" name="preco" value="{{ isset($produto) ? $produto->price : old('preco') }}"
...
<div class="row">
    ...
    <div>
        <input type="number" id="quantidade" name="quantidade" min="1" max="500"
            value="{{ isset($produto) ? $produto->amount : old('quantidade') }}"
